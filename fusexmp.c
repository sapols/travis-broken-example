...
/*for crypto*/
#include "aes-crypt.h" //THIS WAS ADDED
 ...
 * All information within the type file_info is private data */
	//THIS WAS ADDED; I should rename it.
#define ENCFS_DATA ((file_data *) fuse_get_context() -> private_data)
    //THIS COULD BE ADDED TO A .h FILE LIKE IN THE TUTORIAL
typedef struct {
	char *rootdir;
	char *ekey;
} file_data;
...
static int pa5_encfs_getattr(const char *path, struct stat *stbuf)
{
	 ...
	 * update file path */
	char fpath[PATH_MAX]; //RENAME: fullPath
